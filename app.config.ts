...
const TRACKING_PURPOSE =
  'This lets RedactPro show ads that are more relevant to you. Your documents are never uploaded, your data is never sold, and the app works exactly the same either way.';
...
export default ({ config }: ConfigContext): ExpoConfig => {
  ...
  return {
    ...config,
    name: config.name ?? 'RedactPro',
    slug: config.slug ?? 'redactpro',
    plugins: [
