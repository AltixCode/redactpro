...
| | |
|---|---|
| Bundle id / package | `com.altixcode.redactpro` |
| App Store Connect app | `6811548728` (version 1.0, `PREPARE_FOR_SUBMISSION`) |
| In-app purchase | `redactpro_pro_lifetime` — ASC id `6811578497`, state `MISSING_METADATA` |
| RevenueCat project | `proj108442be` |
| RevenueCat apps | iOS `appebe5600637` · Android `appeb68f3b20b` |
...
- **Interstitial**: app/export.tsx — after a redaction is saved to Photos. Pacing lives in `adPolicy.ts`: the first completion
...
1. **Fix the entitlement mismatch — this one ships broken.**
   ...
   rc entitlements create --lookup-key remove_ads \
     --display-name "Pro — Ad-Free & Unlimited" --project-id proj108442be --json --yes
   rc entitlements attach <entitlementId> <productId> --project-id proj108442be --json --yes
   ```
...
4. **Upload the IAP review screenshot.** The in-app purchase
   (`redactpro_pro_lifetime`) is localized and priced at $3.99, and sits at
   `MISSING_METADATA` for want of one screenshot:

   ```bash
   asccli iap-review-screenshot upload --iap-id 6811578497 --file <path-to-png>
   ```
...
2. **RevenueCat — Apple credentials.** `appebe5600637` (the App Store app in project
   `proj108442be`) has no Apple credentials, so App Store purchases cannot be
   validated. This needs an interactive Apple ID sign-in with 2FA:

   ```bash
   rc setup apple appebe5600637
   ```
...
3. **GitHub secrets.** The signing secrets are all present
   ...
   Set each with `gh secret set <KEY> --repo AltixCode/redactpro`. The RevenueCat
   public SDK keys are fetchable — `rc api GET "/projects/proj108442be/apps/appebe5600637/public_api_keys"`
   — the AdMob ones come from step 1.

4. **The legal URLs are wrong and they 404.** `src/config/legal.ts` points at
   `https://www.hushtunnel.com/legal/redactpro-privacy` — **hushtunnel.com**, a leftover from the HushTunnel
...
5. **Play Console — the app record exists.** `com.altixcode.redactpro` is recognised,
...
../scripts/verify-app.sh redactpro com.altixcode.redactpro   # full build + device verification, both platforms
```
